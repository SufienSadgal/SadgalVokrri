...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\linewidth]{RASD/source/images/domain_class_diagram.jpeg}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\linewidth]{RASD/source/images/state_diagrams.jpeg}
...
\begin{enumerate}
    \item Users aim to provide truthful and accurate information when manually reporting bike path conditions
...
\begin{itemize}
    ...
    \item R20: The system shall allow users to visualise available bike paths on a map provided by an external map service.
\end{itemize}

\paragraph{Trip Planning and Route Recommendation}
\begin{itemize}
    \item R9: The system shall allow registered users to plan a bike trip by storing it among planned trips
    \item R10: the system shall allow the user to specify a starting and ending point in order to retrieve all the possible path between them
    \item R11: In the case in which the RU has provided an initial and a final point, the system shall return the possible path ordered based on the score
    * score is an attribute of bike path, specify this on the document 
\end{itemize}

\paragraph{Trip Tracking, History and Performance Analytics}
\begin{itemize}
    \item R6: the system shall allow the registered user to record its trip
    \item R7: The system shall provide registered users with statistics about their cycling performance
    \item R8: the system shall allow the registered user to review all the non deleted past trips
    \item R19: The system shall allow registered users to delete trips from their trip history.
    \item R23: The system shall allow registered users to enable or disable automatic trip tracking. 
\end{itemize}

\paragraph{Reporting, Validation and Publication of Path Information}
\begin{itemize}
    \item R12: the system shall allow registered users to manually produce a report 
    \item R13: the system shall allow registered users to automatically produce a report
    \item R14: the system shall allow registered users to validate reports produced automatically 
    \item R15: the system shall allow registered users to publish manual reports 
    \item R16: the system shall allow registered users to publish validated reports produced automatically
\end{itemize}

\paragraph{Context Enrichment and Notifications}
\begin{itemize}
    \item R17: The system shall notify registered users with planned trips when relevant weather conditions affecting the trip exceed a predefined severity threshold. 
    *modify class diagram with a new attribute severity of weatherInfo =combination of temperature, wind speed
    \item R18: The system shall notify registered users with planned trips when obstacles along the associated path exceed a predefined severity threshold. 
    *we assume that the attribute severity of obstacle is computed as combination of all the reports
    \item R21: The system shall enrich recorded trips with meteorological information retrieved from an external weather service
\end{itemize}
