...
assert G1_VisibleHistoryWellFormed {
    all s: State, u: RegisteredUser | {
        ...
        // visible history contains only completed trips 
        // (i.e., subset of recorded)
...
The assertion \code{G3\_AutoReportsValidatedBeforePublication} formalizes Goal G3 by enforcing three critical constraints on the lifecycle of reports. First, it ensures that every automatically generated report that reaches the Published state has been validated by exactly one registered user. Second, it guarantees that once an automatic report is marked as Discarded, its status remains immutable in all future system states. Third, it enforces immutability for all published reports—both manual and automatic—preventing any further state changes after publication.
The absence of counterexamples gives us confidence about the fact that all three properties consistently hold across every possible execution trace. 

\begin{lstlisting}[caption=Assertion for Goal G3.]
...
\begin{lstlisting}[caption=Predicate showing Trip Lifecycle.]
