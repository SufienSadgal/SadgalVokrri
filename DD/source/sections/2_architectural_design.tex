...
The mobile application is the component responsible for showing the user interface, through which users can interact with the platform. Every interaction of the user with the application triggers a request to the beck-end of the system via HTTPS via the API gateway. Also, this component is responsible for gathering all the required data from the device sensors and GPS, which are directly processed on the server.

Note that, while registered users have full access of all the platform functionalities, non-registered ones have access only to the path exploration service. The mobile application takes into consideration those limitations and displays to non-registered users only the screens they have access to.
