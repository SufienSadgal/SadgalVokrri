...
The Auto Report Generator Worker reacts to trip completion events. It aggregates processed tracking information and generates automatic report drafts in background, which are later reviewed, validated, and published through the Reporting component. (Basically the component 11 create the automatic report draft, the component 7 instead allow to publish reports or to validate automatic reports drafts)


\paragraph{Alert Evaluation Worker}
The Alert Evaluation Worker listens for contextual updates such as weather changes and newly published obstacle reports. It evaluates these events against predefined severity thresholds and determines their impact on planned trips. When critical conditions are detected, it directly triggers alert notifications for affected users. (write here how it works, so:
- notification for weather-> low frequency polling with weather adapter
- notification for obstacles-> event ReportPublished tells the alert component to access to the considered path in the report and check for possible obstancles 
