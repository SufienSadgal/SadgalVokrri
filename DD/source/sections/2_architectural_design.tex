...
This sequence diagram shows the interactions between system components during bike paths browsing. Once the user navigates to the dedicated section in the mobile application, a location request to the device is dispatched. When the user location is returned by the device, the application gets map and weather information from external services, in order to display them. If the user inserts departure and destination points, with attached filters, a request to the server is dispatched, which returns the list of the available bike paths, ranked by score. Otherwise, if the user explores the map, a request by area is sent to the server. In both cases, the map and weather information are updated and the list of bike paths are displayed to the user.

The user can optionally visualize the details of a given path by clicking it,
