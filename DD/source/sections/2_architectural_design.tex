...
\begin{itemize}
    \item Trip Alerts: in a synchronous design, the system would need to repeatedly check obstacle updates in order to detect relevant changes, resulting in unnecessary resource usage and delayed notifications. By adopting an event-driven approach, alerts are triggered only when reports are published, allowing the system to react only when necessary.
...
Before continuing with the analysis of the sequence diagrams, we introduce the following events used by asynchronous components:
\begin{enumerate}
    \item \code{TrackingDataReceived} (\code{TDR}): produced by Tracking Ingestion Endpoint component that transform the data sensed by the GPS of the device into events that will be stored in the bus waiting to be processed.
    \item \code{TrackingDataProcessed} (\code{TDP}): produced by the Tracking Processing Worker after cleaning the data to notify the Automatic Report Generator Worker that the data is ready to be further processed for report compilation.
    \item \code{TripCompleted} (\code{TC}): produced by the Trip Management Service to report that the user has completed a trip. This event is then managed by the Automatic Report Generator that produces a draft of the report, persisted in the database.
    \item \code{ReportPublished} (\code{RP}): produced by the Report Management component upon report publication. It triggers the Alert Management component, which recalculates the severity scores of the paths in the report and notifies users if needed.
\end{enumerate}
